...
const KEY = 'DOWNLOAD_REF_FILE_DOWNLOAD_1.3';
@Injectable({
  providedIn: 'root',
})
export class SlideStaticManagerService {
  audioResource: any;
  ...
  async downloadMediaResource(content: IContent) {
    const base64Data = await this.base64FromPath(content.url);

    const savedFile = await Filesystem.writeFile({
      path: content.name + '.' + content.ext,
      data: base64Data,
  ...
  getMediaResource(content: IContent): Promise<ReadFileResult> {
    if (content.name === 'start-of-array')
      return new Promise((resolve) => resolve({ data: content.url }));
    else
      try {
  ...
  async getMediaResourceAbsoluteURI(content: IContent): Promise<string> {
  ...
  removeMediaResource(content: IContent): void {
    this.md5Deleted(content.name);
  ...
  runAudio(opt: number, number: number, n: string, voz = 'argentina', fromFirebase=false) {
